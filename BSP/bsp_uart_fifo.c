...
*********************************************************************************************************
*	函 数 名: myMemCpy
*	功能说明: 判断结束符，并把COM口接收的数据复制到_pByte，注意数组溢出问题
*	形    参:  无
*	返 回 值: 无
*********************************************************************************************************
*/
...
uint16_t myMemCpy(COM_PORT_E _ucPort,uint8_t *_pByte)
{
    ...
    //-------------------------------------------------//	
    index = _pUart->usRxRead;	
    count = _pUart->usRxCount;	
    ...
    for(i = 0; i < count; i++)
    {
        uct = _pUart->pRxBuf[index];		
        if (++index >= _pUart->usRxBufSize)
        {
            index = 0;
        }
    ...
    //-------------------------------------------------//				
    if(index == 0x5555) // 找到结束符
    {	    
        for(i = 0; i < count; i++)
        {
            *_pByte++ = _pUart->pRxBuf[_pUart->usRxRead];		/* 从串口接收FIFO取数据 */
...
*********************************************************************************************************
*	函 数 名: comMemCpy
*	功能说明: 把COM口接收的数据复制到_pByte，注意数组溢出问题
*	形    参:  无
*	返 回 值: 无
*********************************************************************************************************
*/
...
void USART_SetBaudRate(USART_TypeDef* USARTx, uint32_t BaudRate)
{
    ...
    if (USARTx == USART1)   // WUBO:STM32L无usart6
	{
...
//    USART_InitStructure.USART_Mode = USART_Mode_Rx;		/* 仅选择接收模式 */    
	...
	*/
	...
	/* 打开 GPIO 时钟 */
	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOC |RCC_AHBPeriph_GPIOG, ENABLE);
...
static void ConfigUartNVIC(void)
{
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = USER_PRI;
...
{  
    //---------------------------------------------------------------//        
    //1.使能了接收中断，那么ORE中断也同时被开启了。
    //2.ORE中断只能使用USART_GetFlagStatus(USART1, USART_FLAG_ORE)读到(如果没有使能USART_IT_ERR中断时)
